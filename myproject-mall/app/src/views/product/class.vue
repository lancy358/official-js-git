  ...
  <div>
    ...
    <div class="content">
      <el-table :data="tableData" border style="width: 100%">
        ...
        <el-table-column prop="rank" label="级别" width="80"></el-table-column>
        <el-table-column prop="number" label="商品数量" width="80"></el-table-column>
        <el-table-column prop="unit" label="数量单位" width="80"></el-table-column>
        <el-table-column prop="navigation" label="导航栏" width="120">
          <template slot-scope="scope">
            <el-switch v-model="scope.row.navigation" active-value="1" inactive-value="0"></el-switch>
          </template>
        </el-table-column>
        <el-table-column prop="ismarked" label="是否标记" width="120">
          <template slot-scope="scope">
            <el-switch v-model="scope.row.ismarked" active-value="2" inactive-value="0"></el-switch>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="name" label="设置" width="200">
          <template slot-scope="scope">
            <el-row class="handle" props="handle">
              <el-button size="mini" @click="check(scope.row.id)">查看下级</el-button>
        ...
        <el-table-column prop="handle" label="操作">
          <template slot-scope="scope">
          <el-row class="handle">
            <el-button size="mini" @click="editcheck(scope.row.id)">编辑</el-button>
            <el-button size="mini" style="background:#f56c6c; color:white" @click="deleteClass(scope.row.id)">删除</el-button>
          </el-row>
          </template>
        </el-table-column>
      </el-table>
      ...
      <el-card class="box-card">
        ...
        >
          ...
          <el-form-item label="数量单位" prop="unit">
            <el-input v-model="ruleForm.unit" placeholder="0"></el-input>
          </el-form-item>
          <el-form-item label="排序" prop="sort">
            <el-input v-model="ruleForm.sort" placeholder="0"></el-input>
          </el-form-item>
          <el-form-item label="是否显示" prop="ismarked">
            <el-radio v-model="ruleForm.ismarked" label="1">是</el-radio>
            <el-radio v-model="ruleForm.ismarked" label="2">否</el-radio>
          </el-form-item>
          <el-form-item label="导航栏" prop="navigation">
            <el-radio v-model="ruleForm.navigation" label="1">是</el-radio>
            <el-radio v-model="ruleForm.navigation" label="2">否</el-radio>
          </el-form-item>
...
import { getClassList ,deleteCate,classDetail} from "../../api/api";
export default {
  data() {
    return {
      total: 10,
  ...
  methods: {
    ...
    async getClassFormlist() {
      ...
      if (data.code === 0) {
        ...
        const firstPagetotal = data.data.filter(item => item.pid === 0).length;
        this.total = firstPagetotal;
      } else {
        this.$message.error("获取失败");
      }
    },
    check(id) {
      this.$router.push("/checkProductClass/pid=" + id);
    },
